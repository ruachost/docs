...
```bash cURL
curl -X GET "https://app.notify.ng/api/v2/Template?ApiKey=YOUR_API_KEY&ClientId=YOUR_CLIENT_ID" \
...
const response = await fetch(`https://app.notify.ng/api/v2/Template?${params}`, {
  method: 'GET',
  headers: {
